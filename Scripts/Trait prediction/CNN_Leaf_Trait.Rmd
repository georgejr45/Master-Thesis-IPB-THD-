...
early_stopping <- callback_early_stopping(
  monitor = "val_loss",  # Monitor validation loss
  patience = 500,          # Stop if no improvement for 
  restore_best_weights = TRUE  # Restore weights from the best epoch
)
...
early_stopping <- callback_early_stopping(
  monitor = "val_loss",  # Monitor validation loss
  patience = 500,          # Stop if no improvement for 3 epochs
  restore_best_weights = TRUE  # Restore weights from the best epoch
)
...
Due to the smaller sample size for phosphorous (P), we additionally used data from Pross et al. (2023) collected in the Kreinitz experiment (Germany) to train a CNN. As our samples only contain samples for deciduous species, we only used the available data for deciduous species.
...
```{r }
# Extrapolation on upper data range plot
sla.test_e_h
...
```{r }
# Extrapolation on lower data range plot
sla.test_e_l
